...
export function managePasswords(extId, originEvents) {
    function addIcon(el, clickHandler){
        let span = document.createElement("span")
        span.classList.add("zeropassinjectedspan")
        span.innerHTML = `
        <svg 
        xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink" 
        preserveAspectRatio="xMidYMid" 
        width="30" height="40" viewBox="0 0 602 600" 
        style="transform-origin: 60% 50%;transform: rotate(43deg);width:90%">
        <style>
        path.cls-1{
        ...
        window.setInterval(function(){
            ...
            span.style.top=(document.documentElement.scrollTop + rect.top + rect.height/2 - 20) + "px"
            span.style.left=(document.documentElement.scrollLeft + rect.right -35)+ "px"
        }, 20)
    ...
    function setInput(pwEl, pw){
        pwEl.focus({preventScroll: true})
        if("undefined" !== typeof ClipboardEvent){
            pwEl.dispatchEvent(new ClipboardEvent("paste",{bubbles: true}));
        }
        pwEl.value = pw
        if("undefined" !== typeof InputEvent){
            pwEl.dispatchEvent(new InputEvent("input",{
                bubbles: true,
                inputType: "insertFromPaste"
            }))
        }
        pwEl.dispatchEvent(new Event("change",{
            bubbles: !0
        })),
        pwEl.focus()
    }
  
    document.querySelectorAll(".zeropassinjectedspan").forEach( el => el.remove())
    
    let unEl = document.querySelector('input[autocomplete="username"]')
    let cpwEl = document.querySelector('input[autocomplete="current-password"]')
    let npwEl = document.querySelector('input[autocomplete="new-password"]')
    
    let clickHandler
    if(npwEl != null){
      clickHandler = async ()=>{
        result = await chrome.runtime.sendMessage( {id: "registerPw", username: "", origin: window.location.origin });
        if(typeof result == undefined || result == null || !('password' in result) || typeof  result.password == undefined){
          return
        }

        setInput(npwEl, result.password)
      }
      
      addIcon(npwEl,  clickHandler)
    } else if (unEl != null && cpwEl != null) {
      clickHandler = async ()=>{
        result = await chrome.runtime.sendMessage( {id: "authenticatePw"});
        setInput(unEl, result.username)
        setInput(cpwEl, result.password)
      }
      addIcon(unEl,  clickHandler)
    }

    let webauthnEls = document.querySelectorAll('input[autocomplete*="webauthn"]')
    for(let webauthnEl of webauthnEls){
      addIcon(webauthnEl, async (e) => {
        result = await chrome.runtime.sendMessage( {id: "webauthn", rpId: window.location.origin });
        webauthnEl.value= result.username
      })
    }
  }